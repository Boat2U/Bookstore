...
class User(db_conn.DBConn):
    ...
    def search_title_publisher_isbn(self,title='',publisher='',isbn='',store_id=''):
        result=[]
        if title!='':
        ...
        if isbn!='':
            ...
            isbn=' isbn IS NOT NULL'

        if store_id!='':
            queries=self.session.execute(
            "WITH sub AS (SELECT * FROM book WHERE book_id IN (SELECT book_id FROM store WHERE store_id='%s')) "
            "SELECT title,author,publisher,pub_year,pages,isbn,book_intro,tags,picture FROM sub "
            "%s AND %s AND %s"% (store_id,title,publisher,isbn)).fetchall()
        else:
            queries=self.session.execute(
                "SELECT title,author,publisher,pub_year,pages,isbn,book_intro,tags,picture FROM book "
        ...
        if len(queries)!=0:
            for i in range(len(queries)):
                query=queries[i]
                q_title,q_author,q_publisher,q_pubyear,q_pages,q_isbn,q_bookintro,q_tags,q_picture=query

                try:
                    picture=base64.b64decode(q_picture)
                    result.append(
                        {'title':q_title,'author':q_author,'publisher':q_publisher,'pub_year':q_pubyear,'pages':q_pages,
                        'isbn':q_isbn,'book_intro':q_bookintro,'tags':q_tags,'picture':picture}
                    )
                except:
                    result.append(
                        {'title':q_title,'author':q_author,'publisher':q_publisher,'pub_year':q_pubyear,'pages':q_pages,
                        'isbn':q_isbn,'book_intro':q_bookintro,'tags':q_tags,'picture':''}
                    )
            return 200,result
        else:
            return 527,result

    def search_bookintro_content(self,book_intro='',content='',store_id=''):
        result=[]
    ...
    def search_tag(self,tag='',store_id=''):
        result=[]
        if store_id!='':
            queries=self.session.execute(
                "WITH sub AS (SELECT * FROM book WHERE book_id IN (SELECT book_id FROM store WHERE store_id='%s')) "
    ...
    def search_author_store(self,author='',store_id=''): # 只店铺查找在这儿
        result=[]
        if store_id=='':
            if author=='':
                queries=self.session.execute(
